...
class Scripts(Base):
    ...
    nepse_id = Column(Integer, nullable=True, unique=True, index=True)
...
class ScriptDetails(Base):
    ...
    updated_at = Column(
        DateTime,
        default=lambda: datetime.datetime.now(nepal_tz),
        onupdate=lambda: datetime.datetime.now(nepal_tz),
        nullable=False,
    )
