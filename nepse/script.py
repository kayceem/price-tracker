...
async def get_script_ltp(script:str):
    with get_db() as db:
        script = db.query(Scripts).filter(Scripts.ticker == script).first()
...
async def refresh_script_details():
    with get_db() as db:
        scripts = db.query(Scripts).join(Tracker).all()
    if not scripts:
        return False
    scripts_details = []
    ...
    with get_db() as db:
        for details in scripts_details:
            script = db.query(Scripts).filter(Scripts.id == details.script_id).first()
